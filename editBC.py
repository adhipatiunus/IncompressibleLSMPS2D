...
RAD = 0.1
# %%
node_x, node_y, normal_x_bound, normal_y_bound, n_boundary, index, diameter = generate_particles_singleres(
    xmin, xmax, ymin, ymax, sigma, RAD)
cell_size = r_e * np.max(diameter)
neighbor = neighbor_search_cell_list(
    node_x, node_y, index, cell_size, ymax, ymin, xmax, xmin)
R_e = r_e * sigma
R_s = r_s * sigma
# %%
alphaC = 0.01
...
RHS[idx_begin:idx_end] = - EtaDx[idx_begin:idx_end] @ (conv_2d @ u) \
    ...
    - Ddrag_2d[idx_begin:idx_end] @ (EtaDx @ (u - u_obs) + EtaDy @ (v - v_obs))
...
while T < 10:
    ...
    RHS[idx_begin:idx_end] = 3 / (2 * dt) * (EtaDx[idx_begin:idx_end] @ u_pred + EtaDy[idx_begin:idx_end] @ v_pred) \
        - Ddrag_2d[idx_begin:idx_end] @ (EtaDx @
                                         (u_pred - u_obs) + EtaDy @ (v_pred - v_obs))
...
# %%
visualize(node_x, node_y, p0, diameter, 'initial_pressure.png')
visualize(node_x, node_y, np.sqrt(u_corr**2+v_corr**2),
          diameter, 'initial_pressure.png')
