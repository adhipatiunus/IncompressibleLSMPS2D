...
while T < 1:
    ...
    EtaDxUpwind = sparse.csr_matrix(sparse.vstack(EtaDx))
    EtaDyUpwind = sparse.csr_matrix(sparse.vstack(EtaDy))
    ...
    in_LHS_2d = diff_2d + dt * (conv_2d[n_bound:] + Ddrag_2d[n_bound:])
    # solve for u
    ...
    c_x = np.sum((u_solid) *(h_solid**2)) / (0.5 * (2 * RAD))
    c_y = np.sum((v_solid) *(h_solid**2)) / (0.5 * (2 * RAD))
    CD.append(c_x)
    CL.append(c_y)
    ts.append(T)
    print(' CD = ', c_x)
    ...
    T += dt    
#%%
visualize(node_x, node_y, p, diameter, 'initial_pressure.png')
visualize(node_x, node_y, u, diameter, 'initial_pressure.png')
visualize(node_x, node_y, v, diameter, 'initial_pressure.png')
visualize(node_x, node_y, np.sqrt(u**2+v**2), diameter, 'initial_pressure.png')
#%%
CD = np.array(CD)
CL = np.array(CL)
ts = np.array(ts)
...
# Creating a plot
fig, ax = plt.subplots()
ax.plot(ts, CD, linewidth=2)  # For mesh & manual grid use
...
# Creating a plot
fig, ax = plt.subplots()
ax.plot(ts, CL, linewidth=2)  # For mesh & manual grid use

# Adjust the plot
ax.grid(color='gray', linestyle=':', linewidth=1)

ax.set_title('Lift Coefficient', pad=15, size=20)

ax.set_xlabel("t (s)", style="italic", size=15)
ax.set_ylabel("$C_L$", style="italic", size=15)
